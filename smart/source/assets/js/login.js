...
const validateFormFields = () => {
  const validValuesObj = {
    text: {
      ...
      failureResponse:
        "please provide minimum of two characters",
    },
    number: {
      expected: () => /^[0-9]{2,11}$/,
      failureResponse: "must contain minimum of two and maximum of eleven digits!",
    },
    date: {
      expected: () =>
  ...
  const fieldsToBeValidated = document.querySelectorAll("input, .selGender, textarea");
  const regNum = document.querySelector('.regnum');
  // regNum.parentElement.children[0].textContent("must be six characters long")
//   console.log(fieldsToBeValidated);
  fieldsToBeValidated.forEach((element) => {
    const elementToDisplayError = element.parentElement.children[0];
  ...
  function validate() {
    ...
    if (valTypeStore.expected().test(this.value)) {
      ...
      this.parentElement.children[0].textContent = `${valTypeStore.failureResponse}`;
